...
public class Traffic {
    ...
    public static void main(String[] args) {
        ...
        int [] travel_duration = new int[signals+1];
        ...
        switch (sc.next().charAt(0)){
            case 'y' : {
                ...
                for(int i=0;i<=signals;i++){
                    travel_duration[i] = each_duration;
                }
                break;
            }
            case 'n' : {
        ...
        for(int i=0;i<signals;i++){
            double current_time = (travel_duration[i]*speed) + encount_delay[i];
            double current_total_time = current_time+total_time;
            double remain_time = current_total_time;

            int signal_time = green_light_duration[i]+(red_light_duration[i]*(road_way[i]-1));

                remain_time = remain_time % signal_time;
            

            if(remain_time>green_light_duration[i]){
                int temp_signal_time = signal_time;
                while(signal_time<current_total_time){
        ...
        total_time += travel_duration[signals] * speed;
